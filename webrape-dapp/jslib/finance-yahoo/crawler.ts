...
async function handleLoaded() {
    ...
    const observer = new MutationObserver(() => {
        ...
        if (window.location.href !== previousUrl) {
            ...
            console.log('[finance-yahoo/crawler.ts] The URL has changed from ' + previousUrl + ' to ' + window.location.href);
            // Update the previous URL
            previousUrl = window.location.href;
            floatDiv.innerHTML = "[finance-yahoo/crawler.ts] This is a float div with scraped web data in json from url=" + window.location.href;
        }
    });
    ...
    waitForElm<HTMLDivElement>("#my-float-div").then(async elm => {
        console.log("[finance-yahoo/crawler.ts] Element(my-float-div) is ready and emit(DOMContentLoadedxxx): ", $(elm).text());

        var pp = await emit("DOMContentLoadedxxx", { loggedIn: true, token: 'authToken@waitForElm<HTMLDivElement>("#my-float-div")' });


        console.log("[finance-yahoo/crawler.ts] after emit(DOMContentLoadedxxx)",pp);


        const unlisten = await listen("BackendEventxyz", (event) => {
            console.log("[finance-yahoo/crawler.ts] listen got BackendEventxyz@my-float-div ", event)
        })
    ...
    waitForElm<HTMLDivElement>("#jztable table").then(elm => {
        console.log("[finance-yahoo/crawler.ts] extractJjjzHistoryData for Element(jztable) is ready: ", $(elm).text());
    ...
    await invoke("plugin:fund_eastmoney|open_link", { "url": "open_link from /plugins/fund-eastmoney/js/finance-yahoo/crawler.ts" });
};
